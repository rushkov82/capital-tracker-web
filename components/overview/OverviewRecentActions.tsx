...
type OverviewRecentActionsProps = {
  operations: Operation[];
  formatNumber: (value: number) => string;
};

function getOperationTitle(operation: Operation) {
  if (operation.type === "expense") return "Вывод";
  if (operation.type === "adjustment") return "Корректировка";
  return "Пополнение";
}

export default function OverviewRecentActions({
  operations,
  formatNumber,
}: OverviewRecentActionsProps) {
  return (
    <section className="app-card">
      <div className="space-y-4">
        <div>
          <h2 className="app-card-title">Последние действия</h2>
          <div className="app-text-small mt-1">
            Последние изменения в капитале
          </div>
        </div>
